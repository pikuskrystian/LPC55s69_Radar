...
void LCD_GramRefresh ()
{
	LCD_SetHome();
	LCD_DC_1;
	SPI_Write_Words(SPI_base, L_FrameBuffer, LCD_WIDTH*LCD_HEIGHT);
}
...
void LCD_Puts(uint16_t x, uint16_t y, char *text, uint16_t color)
{
	...
	while(text[n]) {
		...
		c_offset=text[n] - ' ';
		//------ UTF-8 polskie znaki ------
		if(text[n]==0xC3 && text[n+1]==0xB3){// Ó
			c_offset=98;
			n++;
		}
		if(text[n]==0xC3 && text[n+1]==0x93){//ó
			c_offset=97;
			n++;
		}
